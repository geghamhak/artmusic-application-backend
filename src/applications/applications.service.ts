...
export class ApplicationsService {
  ...
  async create(createApplicationDto: CreateApplicationDto) {
    try {
      ...
      const {
        isFree, firstComposition, secondComposition, leaderFirstName,
        participants, participantTypeId, countryId, leaderLastName,
        nominationId, isOnline, phoneNumber, quantity, regionName, schoolName,
        uploadedAudio, uploadedImages, totalDuration,
        videoLinks, email, regionId, schoolId} = createApplicationDto;
      ...
      application.participantVideoLinks = this.participantVideoLinksService.create(videoLinks);
      application.participants = this.participantsService.create(participants);
      application.participantRecordings = this.participantRecordingsService.create(uploadedAudio);
      application.participantDocuments = this.participantDocumentsService.create(uploadedImages);
      application.nomination = { id: nominationId } as Nomination;
      application.participantType = { id: participantTypeId } as ParticipantType;
  ...
  async update(id: number, updateApplicationDto: UpdateApplicationDto) {
    ...
    const scoringSystem = await this.scoringSystemService.determinePlaceByScore(score, application.festival.festivalType.id);
